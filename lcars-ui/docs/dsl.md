...
The nine bundled IDs (`galaxy`, `nemesis`, `tng`, `outpost`, `cardassian`, `klingon`,
`romulan`, `ferengi`, and `gruvbox`) are always available. To ship a project-specific
theme, add a `*.toml` file under the project-root `themes/` directory. `App(themes_dir=...)`
selects a different directory; relative paths resolve from the CLI-discovered project root
(or the current working directory for a directly run Python script). A missing directory
means bundled themes only.

The filename stem is the theme ID, must match `[a-z0-9][a-z0-9_-]*`, and cannot replace a
bundled ID. Each custom theme extends one bundled ID only; custom themes do not extend
other custom themes. The following complete file is also the shape generated by
`lcars new` as `themes/bridge-night.toml`:
...
Every value is optional except `version`, `label`, and `extends`; omitted colors and fonts
inherit from `extends`. Color values are six-digit hex values. The color keys above and the
three font keys are the entire allowlist. Font values are family-name fallback lists, not
URLs or font files. Unknown keys, malformed TOML, unsafe font CSS, invalid colors, an
unknown base, or a selected missing ID make `lcars check`, `build_manifest()`, and startup
fail with the source file and field.

Use an ID from the bundled set or this directory in `app.config(theme=...)`,
`ctx.set_theme(...)`, or `lcars_ui.set_theme(...)`. The Options page reads the manifest
catalogue dynamically, saves a viewer's choice locally, and safely returns to the app
default if its saved file later disappears. Themes deliberately do not provide arbitrary
CSS, geometry/spacing controls, a browser editor, downloads, or font-file hosting.
