...
The recreations are declared in `lcars-ui/examples/canon_recreation/app.py` as chrome-free authored compositions using the public `lcars_ui` Python DSL. Their measured rectangles become proportional composition tracks, so the original topology is preserved without using the source image at runtime. Dense content remains native UI: the seismic waveform is 108 structural bars, the periodic table is 75 data-tile controls, and the holodeck selection bank is 18 controls.

The capture script launches the normal LCARS server and React frontend, rejects image requests and forbidden raster-bearing DOM/CSS, and takes the screenshots at each reference frame's native dimensions. It also rejects adaptive shell output and verifies each authored stage, dense-item count, and representative geometry anchors within four pixels.
