...
test results, permission probes, and the like). UNKNOWN is not rendered as a warning. When
`on_escalate="EXACT"`, the call returns `True` only for the escalation action.

```python
escalate = advanced.tri_state(
    {
...
not returned to application code.
