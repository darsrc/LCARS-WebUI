...
|---|---|---|---|
| `lcarsPage()` | `lcars.config()` + `lcars.page()` + the strict page shell and code-rendered band/lane geometry | Partial / divergent | WebUI adds persistent shell concerns (transport/action/nav rails); `normalize_manifest_for_strict` compiles generic groups into strict structural containers before frontend rendering. |
| `lcarsHeader()` | `lcars.header()` + the `lcars_header` structural widget | Partial | Present as a structural primitive and passed through unchanged by strict normalization. |
| `lcarsBox()` | `lcars.box()` + the `lcars_box` structural widget, including `children` and left/right input regions | Partial | Explicit boxes pass through strict normalization; bare input and data groups are also auto-paneled into `lcars_box` containers. Product controls inside use LCARS-native button, segment, option-stack, input, and check geometry. |
| `lcarsSweep()` | `lcars.sweep()` + the `lcars_sweep` structural widget | Partial / divergent | Strict normalization injects a page-title sweep for titled pages. The current language record does not establish reference-style `left_width` dual-content semantics. |
| `lcarsBracket()` | `lcars.bracket()` + the `lcars_bracket` structural widget | Divergent | WebUI bracket orientation is `left|right|both`; strict normalization uses `both` for mixed groups and `left` for single widgets, rather than the reference's top/bottom bracket pair. |
...
| Containers framing rather than composing | `lcarsBox` side/input behavior is explicit; content and side panels have specific structural rules tied to elbows/sides. | `normalize_manifest_for_strict` auto-panels bare input, data, mixed, and single-widget groups into `lcars_box` or `lcars_bracket`; explicit structural containers pass through unchanged. | Composition inferred from generic group categories can still make containers wrappers around pre-existing layout intent rather than the sole owners of composition semantics. |
...
- The reference package does not define an app-level shell equivalent to WebUI's transport/action/navigation rails. Whether those shell concerns remain in strict mode is a product decision, not resolved by reference parity alone.
